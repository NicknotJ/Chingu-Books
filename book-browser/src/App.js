...
class App extends React.Component {
  ...
  removeFavorites = (book) => {
    console.log('help');
    //This will remove a book from the favorites list. Will probably need to loop through looking for an id #
  }
  
  buildBookShelf = (func) => {
    return this.state.bookShelf ? this.mapBookList(this.state.favorites, func) : '';
  }

  buildSearchList = (func) => {
    if(this.state.bookShelf) return;
    if(this.state.error) return <p>{this.state.searchResult}</p>
    if(this.state.searchResult){
      console.log('here are the searchResults');
      console.log(this.state.searchResult);
      return this.state.searchResult.items ? this.mapBookList(this.state.searchResult.items, func) : <p>Google did not return anything</p>
      //need something to return if there are no searchResults
    } else {
      return <p>Your Search Results Will Display Here</p>;
    }
  }

  mapBookList = (arrayOfBooks, func) => {
      console.log('mapBookList ran!');
      console.log(arrayOfBooks)
      let books = [];
      //Will need to handle issues where the server does not respond
      if(arrayOfBooks){
        let end = Math.min(arrayOfBooks.length, 10);
        for (let x = 0; x < end; x++){
          ...
          <ul>
            {books.map(function(book, index){
              return <li className="book" key={index}>
                       <Book book={book} onClick={(book) => func(book)} />
                    </li>
            })}
          </ul>
      )
  ...
  render(){
    ...
    return (
      <main className="App">
        ...
        {/*<Book book={results} onClick={this.addFavorites}/> this works while the books above doesn't */}
        <section className="bookResults">
          {this.loading()}
          {this.buildSearchList(this.addFavorites)}
          {this.buildBookShelf(this.removeFavorites)}
